...
export default function PhotosScreen({ navigation }) {
  ...
  const handleCreateAlbum = () => {
    if (!user) {
      showAlert('Not signed in', 'You need to be signed in to create albums.');
      return;
    }
    if (!familyId) {
  ...
  const handleDeleteAlbum = (album) => {
    ...
    showConfirm('Delete Album and all photos?', 'Are you sure you want to delete this?', {
      onCancel: () => {
        longPressFlag.current = false;
      },
      onConfirm: async () => {
    ...
    if (Platform.OS === 'web') {
      return (
        <View style={styles.albumCard}>
          <TouchableOpacity style={styles.albumOpenButton} onPress={() => handleAlbumPress(item)} activeOpacity={0.7}>
...
const styles = StyleSheet.create({
  ...
  webDeleteButton: Platform.OS === 'web' ? { zIndex: 2, elevation: 2, cursor: 'pointer' } : {},
  deleteButtonText: {
    color: colors.error,
